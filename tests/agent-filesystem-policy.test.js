...
describe('agent filesystem allowlist', () => {
  ...
  it('allows Windows shell-style absolute paths inside the repo', async () => {
    const p = mkProject();
    const normalizedRepo = p.repo.replaceAll('\\', '/');
    const normalizedFile = path.join(p.repo, 'repo.txt').replaceAll('\\', '/');
    const mntPath = normalizedFile.replace(/^([A-Za-z]):/, (_, drive) => `/mnt/${drive.toLowerCase()}`);
    const workspaceAlias = '/workspace/repo.txt';

    assert.match(executeRead({ file_path: mntPath }, p.repo, p.allowedWorker), /repo ok/);
    assert.match(executeRead({ file_path: workspaceAlias }, p.repo, p.allowedWorker), /repo ok/);

    const bashResult = await executeTool('Bash', { command: `cat ${JSON.stringify(mntPath)} && cat ${JSON.stringify(workspaceAlias)}` }, p.repo, 0, {}, null, null, p.allowedWorker);
    assert.match(bashResult, /repo ok/);
    assert.ok(normalizedRepo.length > 0);
  });
});
