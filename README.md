...
- 支持美术 Agent 直接分配图像生成模型产出图片，系统自动将模型返回的图片保存到仓库，无需 MCP 或外部工具中转。没有生图模型的环境下自动降级为文字方向指导。
...
- 美术直出图片：美术 Agent 可直接分配图像生成模型（如 Gemini Image），模型返回的图片自动保存到仓库 `assets/generated/`，供程序和 UI 直接引用。没有生图模型时自动降级为文字方向指导。
...
- 视觉理解、素材判断、图像生成：优先给美术或视觉岗位分配多模态模型。如果当前模型目录中有带 `image` / `generation` 标签的模型（如 `Google/gemini-3-pro-image-preview`），可以直接把美术 Agent 的 `model` 设为该模型 ID，Agent 对话时产出的图片会自动存入 `assets/generated/`。如果没有可用的生图模型，美术 Agent 仍可使用通用高档位模型做文字层面的视觉方案和风格指南。
