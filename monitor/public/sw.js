// Service Worker for TheBotGameCompany PWA notifications
self.addEventListener('install', (e) => self.skipWaiting());
self.addEventListener('activate', (e) => e.waitUntil(self.clients.claim()));

// Listen for push events from server
self.addEventListener('push', (event) => {
  const data = event.data ? event.data.json() : { title: 'TheBotGameCompany', body: 'New event' };
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'TheBotGameCompany', {
      body: data.body,
