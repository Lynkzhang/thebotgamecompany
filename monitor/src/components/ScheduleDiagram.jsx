...
export function getAgentEntries(schedule) {
  ...
  for (const step of steps) {
    if (Array.isArray(step?._parallel)) {
      for (const parallelStep of step._parallel) {
        for (const [key, value] of Object.entries(parallelStep || {})) {
          if (key !== 'delay') entries.push([key, value])
        }
      }
      continue
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
      ...
      {directives.includes('exam_pass') && (
        <MetaCard label="⚖️ Final Review Approved" color="#10b981">
          {examPass?.message && (
      ...
      {directives.includes('exam_fail') && (
        <MetaCard label="⚖️ Final Review Rejected" color="#ef4444">
          {examFailFeedback && (
