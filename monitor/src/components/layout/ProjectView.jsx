...
import { Users, User, Sparkles, Settings, ScrollText, RefreshCw, Pause, Play, RotateCcw, Save, GitPullRequest, ArrowLeft, Github, Bell, ChevronDown, Lock, Unlock, FolderOpen } from 'lucide-react'
...
export default function ProjectView({
  ...
  const controlAction = async (action) => {
    ...
    try {
      ...
      else showToast(`操作“${action}”失败`)
    } catch (err) { showToast(`操作“${action}”失败：${err.message}`) }
  }
  ...
  const openProjectFolder = async () => {
    if (!selectedProject) return
    try {
      const res = await authFetch(projectApi('/open-folder'), { method: 'POST' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error(data.error || 'Failed to open project folder')
      }
    } catch (err) {
    ...
    <div className="flex-1 min-w-0 bg-neutral-50 dark:bg-neutral-950 p-6 max-w-screen-2xl mx-auto overflow-y-auto">
      <div>
        ...
        <div className="mb-6 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 pl-8 sm:pl-0 shrink-0">
              ...
              <button
                ...
                title={isWriteMode ? '当前为写入模式，点击可锁定' : '当前为只读模式，点击可解锁'}
              >
              ...
              {repoUrl && (
                <a href={repoUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 text-neutral-600 dark:text-neutral-300 inline-flex items-center" title="在 GitHub 中打开">
                  <Github className="w-4 h-4" />
                </a>
              )}
              {isWriteMode && (selectedProject.paused ? (
                <button onClick={() => controlAction('resume')} className="p-1.5 rounded bg-green-500 hover:bg-green-600 text-white transition-colors" title="恢复项目">
                  <Play className="w-4 h-4" />
                </button>
              ) : (
                <button onClick={() => controlAction('pause')} className="p-1.5 rounded bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 text-neutral-600 dark:text-neutral-300 transition-colors" title="暂停项目">
                  <Pause className="w-4 h-4" />
                </button>
              ))}
              {isWriteMode && <button onClick={openBootstrapModal} className="p-1.5 rounded bg-red-500 hover:bg-red-600 text-white transition-colors" title="重置工作区">
          ...
          {error && <Badge variant="warning">错误：{error}</Badge>}
        </div>

        {selectedProject && projectLoading && (
          <div className="flex items-center justify-center py-20">
            <RefreshCw className="w-6 h-6 animate-spin text-neutral-400" />
            <span className="ml-3 text-neutral-500 dark:text-neutral-400">项目加载中...</span>
          </div>
        )}
        ...
        {selectedProject && !projectLoading && (
          <>
              ...
              <div className="mb-4 p-4 rounded-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <Badge variant={
                    ...
                    {selectedProject.phase === 'planning' ? '规划 / 执行制作人' : selectedProject.phase === 'implementation' ? (selectedProject.isFixRound ? '修复 / PM' : '实现 / PM') : selectedProject.phase === 'verification' ? '验证 / QA 负责人' : selectedProject.phase}
                  </Badge>
                  {selectedProject.milestoneCyclesBudget > 0 && selectedProject.phase === 'implementation' && (
                    <span className="text-xs font-mono text-neutral-500 dark:text-neutral-400 ml-auto">
                      {selectedProject.milestoneCyclesUsed}/{selectedProject.milestoneCyclesBudget} 轮
                    </span>
                  )}
                </div>
                {selectedProject.milestoneCyclesBudget > 0 && selectedProject.phase === 'implementation' && (
            ...
            {/* Logs */}
            <Card className="mt-4">
              <CardHeader><CardTitle className="flex items-center gap-2"><ScrollText className="w-4 h-4" />编排器日志</CardTitle></CardHeader>
              <CardContent>
                <div 
                  ref={logsRef}
                ...
                >
                  {logs.length === 0 ? <p className="text-neutral-500">暂无日志</p> : logs.map((line, idx) => (
