        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          ...
          {toHuman.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-[11px] font-semibold text-red-600 dark:text-red-400 uppercase tracking-wide">需要你处理</span>
              </div>
              {toHuman.map(issue => (
          ...
          {fromHuman.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide">你发起的请求</span>
              </div>
              {fromHuman.map(issue => (
                <IssueRow key={issue.id} issue={issue} openIssueModal={openIssueModal} />
              ))}
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-neutral-400 dark:text-neutral-500 text-center py-4">
              暂无{filter === 'all' ? '' : filter === 'open' ? '进行中的' : '已关闭的'}人工介入事项
            </p>
          )}
        </div>
