  ...
  return (
    <div className="mb-0.5">
      <div className="flex items-center gap-2">
        ...
        {!report._live && report.timed_out === 1 && <Timer className="w-3 h-3 text-orange-500 shrink-0" title="超时" />}
        ...
        <div className="pl-7 mt-0.5 space-y-1">
          ...
          {(report.input_tokens > 0 || report.output_tokens > 0 || report.cache_read_tokens > 0) && (
            <div className="text-[10px] text-neutral-400 dark:text-neutral-500 leading-relaxed">
               {report.input_tokens > 0 && <span>{formatTokens(report.input_tokens)} 新输入</span>}
               {report.cache_read_tokens > 0 && <span className="ml-1">{formatTokens(report.cache_read_tokens)} 缓存</span>}
               {report.output_tokens > 0 && <span className="ml-1">{formatTokens(report.output_tokens)} 输出</span>}
            </div>
          )}
        </div>
      )}
...
export default function AgentReportsCard({
  ...
  return (
    <DashboardWidget
      icon={MessageSquare}
      title="Agent 报告"
      headerRight={<span className="text-sm font-normal text-neutral-500 dark:text-neutral-400">已加载 {comments.length} 条</span>}
    >
            ...
            <>
              ...
              >
                ...
                <div className="text-xs text-neutral-500 dark:text-neutral-400 break-words leading-relaxed pl-7 italic">
                  运行中...（{liveAgentLog.log.length} 条日志）
                </div>
              </div>
              {comments.length > 0 && <Separator className="my-1" />}
            </>
          )}
          {comments.length === 0 && !commentsLoading && !liveAgentLog && <p className="text-sm text-neutral-400 dark:text-neutral-500 text-center py-4">暂无报告</p>}
