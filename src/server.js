#!/usr/bin/env node
/**
 * TheBotGameCompany - Multi-project AI Agent Orchestrator
 * 
...
function summarizeMetaOnlyResponse(text) {
  if (!text) return '';
  if (/<!--\s*SCHEDULE\s*-->/.test(text)) {
    const scheduleMatch = text.match(/<!--\s*SCHEDULE\s*-->\s*([\s\S]*?)\s*<!--\s*\/SCHEDULE\s*-->/);
    if (scheduleMatch) {
      try {
        const schedule = JSON.parse(scheduleMatch[1]);
        if (Array.isArray(schedule) && schedule.every(step => step && typeof step === 'object' && Object.keys(step).every(key => key === 'delay'))) {
          return 'delay-only schedule';
        }
        return 'schedule emitted';
      } catch {
        return 'schedule emitted';
      }
    }
  ...
  if (/<!--\s*MILESTONE\s*-->/.test(text)) return 'milestone emitted';
  if (/<!--\s*PROJECT_COMPLETE\s*-->/.test(text)) return 'project completion emitted';
  if (/<!--\s*CLAIM_COMPLETE\s*-->/.test(text)) return 'claim complete emitted';
  if (/<!--\s*VERIFY_PASS\s*-->/.test(text)) return 'verification pass emitted';
  if (/<!--\s*VERIFY_FAIL\s*-->/.test(text)) return 'verification failure emitted';
  if (/<!--\s*EXAM_PASS\s*-->/.test(text)) return 'examination pass emitted';
  if (/<!--\s*EXAM_FAIL\s*-->/.test(text)) return 'examination failure emitted';
  return '';
}
...
class ProjectRunner {
  constructor(id, config) {
    ...
    this.epochCount = 0;   // Epochs: planning → implementation → verification → planning loops
    ...
    // Phase state machine: planning | implementation | verification | examination
    this.phase = 'planning';
  ...
  setState(patch, { save = true } = {}) {
    ...
    if (normalizedPatch.phase) normalizedPatch.phase = normalizePhaseName(normalizedPatch.phase);
    // Apply patch
    ...
    // 3. Phase reset: when entering planning phase, clear implementation/verification state
    if (normalizedPatch.phase === 'planning' && !normalizedPatch.milestoneDescription) {
      // Don't clear milestone info if it wasn't explicitly set — producer needs context
    }
  ...
  get workerSkillsDir() {
    ...
    const repoLegacyDir = this.repoWorkerSkillsDir;
    if (fs.existsSync(repoLegacyDir)) {
      fs.mkdirSync(dir, { recursive: true });
      for (const file of fs.readdirSync(repoLegacyDir)) {
        if (!file.endsWith('.md')) continue;
        const fromPath = path.join(repoLegacyDir, file);
  ...
  get repoWorkerSkillsDir() {
    return path.join(this.path, 'skills', 'workers');
  }

  getWorkerSkillPath(agentName) {
    const normalizedName = normalizeAgentName(agentName);
    return path.join(this.workerSkillsDir, `${normalizedName}.md`);
  }

  isParallelSafeAgent(agent) {
    if (!agent || agent.isManager) return false;
    const role = String(agent.role || '').trim();
    if (!role) return false;
    if (PARALLEL_BLOCKED_ROLE_RE.test(role)) return false;
    return PARALLEL_SAFE_ROLE_RE.test(role);
  }

  _refreshActiveAgentSummary() {
    const entries = Object.values(this.activeAgents || {});
    if (entries.length === 0) {
      this.currentAgent = null;
      this.currentAgentProcess = null;
      this.currentAgentStartTime = null;
    ...
    if (entries.length === 1) {
      ...
      this.currentAgent = only.name;
      this.currentAgentProcess = only.process;
      this.currentAgentStartTime = only.startTime;
      this.currentAgentLog = only.log;
      this.currentAgentModel = only.model;
      this.currentAgentCost = only.cost;
      this.currentAgentUsage = only.usage;
      this.currentAgentKeyId = only.keyId;
      this.currentAgentVisibility = only.visibility;
      return;
    }

    this.currentAgent = entries.map(entry => entry.name).join(', ');
    ...
    this.currentAgentStartTime = Math.min(...entries.map(entry => entry.startTime || Date.now()));
    this.currentAgentLog = entries
      .flatMap(entry => (entry.log || []).map(item => ({ ...item, msg: `[${entry.name}] ${item.msg}` })))
      .sort((a, b) => a.time - b.time)
      .slice(-500);
    this.currentAgentModel = `${entries.length} agents running`;
    this.currentAgentCost = entries.reduce((sum, entry) => sum + (entry.cost || 0), 0);
    ...
    this.currentAgentKeyId = null;
    this.currentAgentVisibility = { mode: 'parallel', issues: [] };
  }

  _registerActiveAgentRun(runState) {
    this.activeAgents[runState.name] = runState;
    this._refreshActiveAgentSummary();
    broadcastStatusUpdate(this.id);
  }

  _clearActiveAgentRun(agentName) {
    delete this.activeAgents[agentName];
    this._refreshActiveAgentSummary();
    broadcastStatusUpdate(this.id);
  }
  ...
  getAgentDetails(agentName) {
    ...
    let skillPath = this.getWorkerSkillPath(agentName);
    let isManager = false;
  ...
  async createIssue(title, body = '', creator = 'human', assignee = null) {
    ...
    try {
      ...
      const normalizedCreator = normalizeAgentName(creator || 'human');
  ...
  getStatus() {
    const activeAgents = Object.values(this.activeAgents || {}).map(entry => ({
      ...
      visibility: entry.visibility || { mode: 'full', issues: [] },
      runtime: entry.startTime ? Math.floor((Date.now() - entry.startTime) / 1000) : null,
    }));
    return {
      id: this.id,
      path: this.path,
  ...
  // Kill Epoch: terminate everything + force back to planning
  killEpoch() {
    log(`🔴 Kill Epoch: terminating agent, clearing schedule, returning to producer planning`, this.id);
  ...
  scheduleHasAgentSteps(schedule) {
    return !!(schedule && schedule._steps && schedule._steps.some(step => {
      if (Array.isArray(step?._parallel)) return step._parallel.length > 0;
      return Object.keys(step).some(key => key !== 'delay');
    }));
  }

  _validateParallelStep(parallelEntries, freshWorkers) {
    const names = [];
    for (const step of parallelEntries) {
      const workerName = Object.keys(step || {}).find(key => key !== 'delay');
      if (!workerName) return { ok: false, error: 'Parallel step must contain exactly one worker entry' };
      const worker = freshWorkers.find(w => w.name.toLowerCase() === workerName.toLowerCase());
      if (!worker) return { ok: false, error: `Worker "${workerName}" not found in workspace/skills/workers` };
      if (!this.isParallelSafeAgent(worker)) {
        return { ok: false, error: `Worker "${workerName}" (${worker.role || 'unknown'}) is not allowed in _parallel; program/code roles must remain serial` };
      }
      names.push(workerName.toLowerCase());
    }
    if (new Set(names).size !== names.length) {
      return { ok: false, error: 'Parallel block contains duplicate worker names' };
    }
    return { ok: true };
  }

  async _runScheduledWorker(worker, config, task, vis) {
    let total = 0;
    let failures = 0;
    const maxRetries = 2;
    let attempt = 0;
    ...
    return { total, failures, succeeded };
  }

  async executeSchedule(schedule, config) {
    if (!schedule || !schedule._steps) return { total: 0, failures: 0 };

    const hasAgentSteps = this.scheduleHasAgentSteps(schedule);
    if (!hasAgentSteps) {
      log('Ignoring schedule with only delay steps', this.id);
    ...
    for (const step of schedule._steps) {
      ...
      if (Array.isArray(step._parallel)) {
        const pendingSteps = step._parallel.filter(parallelStep => {
          const parallelName = Object.keys(parallelStep || {}).find(key => key !== 'delay');
          return parallelName && !this.completedAgents.includes(parallelName.toLowerCase());
        });
        if (pendingSteps.length === 0) continue;

        const validation = this._validateParallelStep(pendingSteps, freshWorkers);
        if (!validation.ok) {
          log(validation.error, this.id);
          failures += pendingSteps.length;
        ...
        log(`Running parallel block: ${pendingSteps.map(parallelStep => Object.keys(parallelStep).find(key => key !== 'delay')).join(', ')}`, this.id);

        const results = await Promise.all(pendingSteps.map(async parallelStep => {
          const parallelName = Object.keys(parallelStep).find(key => key !== 'delay');
          const worker = freshWorkers.find(w => w.name.toLowerCase() === parallelName.toLowerCase());
          const value = parallelStep[parallelName];
          const task = typeof value === 'string' ? value : value.task || null;
          const vis = this._parseVisibility(value, task);
          return this._runScheduledWorker(worker, config, task, vis);
        }));
      ...
      if (!worker) {
        const error = `Worker "${name}" not found in workspace/skills/workers`;
        log(error, this.id);
        failures++;
        total++;
        continue;
      ...
      const task = typeof value === 'string' ? value : value.task || null;
      const vis = this._parseVisibility(value, task);
      const result = await this._runScheduledWorker(worker, config, task, vis);
      total += result.total;
      failures += result.failures;
    }
  ...
  async runLoop() {
    while (this.running) {
      ...
      if (this.phase === 'planning') {
        const producer = managers.find(m => m.name === 'producer');
        if (producer) {
          // Build situation context for producer
          let situation = '';
          if (this.examinationFeedback) {
            situation = `> **Situation: Project completion rejected by final review**\n> ${this.examinationFeedback}\n\n`;
          } else if (!this.milestoneDescription) {
            situation = '> **Situation: Project Just Started**\n\n';
          } else if (this.verificationFeedback === '__passed__') {
            situation = '> **Situation: Milestone Verified Complete**\n> Previous milestone was verified by the QA team.\n\n';
          } else {
            situation = `> **Situation: Implementation deadline missed**\n> PM team used ${this.milestoneCyclesUsed}/${this.milestoneCyclesBudget} cycles without completing the milestone.\n\n`;
          }

          const result = await this.runAgent(producer, config, null, situation);
          cycleTotal++;
          if (!result || !result.success) cycleFailures++;

          // Parse schedule and milestone from producer response
          let schedule = null;
          if (result && result.resultText) {
            ...
            if (completeMatch) {
              try {
                ...
                if (success) {
                  ...
                  log(`🧪 PROJECT COMPLETE claimed — routing to final review: ${message}`, this.id);
            ...
            if (fs.existsSync(path.join(this.agentDir, 'STOP'))) {
              log(`STOP file detected — pausing project`, this.id);
                this.setState({ isPaused: true, pauseReason: 'Project stopped by producer' });
              continue;
            }
          }
          ...
          if (schedule) {
            if (this.scheduleHasAgentSteps(schedule)) {
      ...
      // ===== PHASE: IMPLEMENTATION (PM + execution team) =====
      ...
      // ===== PHASE: VERIFICATION (QA lead + reviewers) =====
        ...
        const qaLead = managers.find(m => m.name === 'qa_lead');
        if (qaLead) {
          const qaContext = `> **Milestone to verify:** ${this.milestoneDescription}\n\n`;

          const result = await this.runAgent(qaLead, config, null, qaContext);
          cycleTotal++;
          if (!result || !result.success) cycleFailures++;
      ...
      // ===== PHASE: EXAMINATION (final review) =====
      else if (this.phase === 'examination') {
        const finalReview = managers.find(m => m.name === 'final_review');
        if (finalReview) {
          const finalReviewContext = `> **Final completion claim:** ${this.pendingCompletionMessage || 'Project claimed complete'}\n> **Evaluate the entire project, not just the human\'s explicit goal.** Audit correctness, completeness, maintainability, artifacts, tests, docs, and obvious risks.\n\n`;
          const result = await this.runAgent(finalReview, config, null, finalReviewContext, { mode: 'blind', issues: [] });
          cycleTotal++;
          if (!result || !result.success) cycleFailures++;
          ...
          if (result && result.resultText) {
            ...
            if (failMatch) {
              try {
                ...
                failData = { feedback: 'Final review rejected project completion, but the response could not be parsed.' };
              }
            }
          }
          ...
          if (decision === 'pass') {
            ...
            log(`🏁 PROJECT COMPLETE (validated by final review): ${message}`, this.id);
            ...
            if (issues.length === 0) {
              issues = [{
                title: 'Final review rejected project completion',
                body: rawFeedback || failData?.feedback || failData?.summary || 'Final review did not issue EXAM_PASS, so the completion claim was rejected.',
              }];
            }
            ...
            for (const issue of issues) {
              ...
              try {
                const created = await this.createIssue(issue.title, issue.body || '', 'final_review');
                createdIssueIds.push(created.issueId);
              } catch (e) {
                log(`Final review issue creation failed: ${e.message}`, this.id);
              }
            }
            const feedback = failData?.feedback || failData?.summary || rawFeedback || 'Final review rejected the project completion claim.';
            this.setState({
              phase: 'planning',
            ...
            log(`❌ Final review rejected project completion — returning to producer`, this.id);
            broadcastEvent({ type: 'phase', project: this.id, phase: 'planning', title: this.milestoneTitle || 'Replanning after final review rejection' });
          }
        }
      }
    ...
    try {
      ...
      if (agent.name !== 'final_review') {
        if (visMode === 'full') {
          ...
          sharedRules += '\n> **You are final review.** You must not read communication history, issue tracker contents, or any workspace notes, including your own. Evaluate only the repository, tests, artifacts, and your own fresh inspection.\n\n---\n\n';
      }
  ...
  _postProcessAgentRun(agent, config, runState, { resultText, cost, durationMs, killedByTimeout, exitCode, rawOutput, apiSuccess, usage }) {
    ...
    const summary = resultText
      ? (stripMetaBlocks(resultText).slice(0, 500).replace(/\n+/g, ' ').trim() || summarizeMetaOnlyResponse(resultText))
      : '';
    broadcastEvent({ type: 'agent-done', project: this.id, agent: agent.name, success, summary });
    this._clearActiveAgentRun(agent.name);
  ...
  async runAgent(agent, config, mode = null, task = null, visibility = null) {
    ...
    this._registerActiveAgentRun(runState);
    const processRef = {
      kill: () => runAbortController.abort(),
    };
    runState.process = processRef;
    ...
    if (!skillContent) {
      const skillPath = agent.isManager
        ? path.join(ROOT, 'agent', 'managers', `${agent.name}.md`)
        : this.getWorkerSkillPath(agent.name);
    ...
    const result = await runAgentWithAPI({
      ...
      log: (msg) => {
        ...
        runState.log.push({ time: Date.now(), msg });
...
function loadProjects() {
  ...
  try {
    if (!fs.existsSync(projectsPath)) {
      const defaultConfig = `# TheBotGameCompany - Project Registry
projects:
  # Example:
  # m2sim:
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathParts[0] === 'api' && pathParts[1] === 'projects' && pathParts[2]) {
    ...
    if (req.method === 'POST' && subPath === 'open-folder') {
      if (!requireWrite(req, res)) return;
      try {
        const targetPath = runner.path;
    ...
    if (req.method === 'GET' && subPath === 'agent-log') {
      ...
      res.end(JSON.stringify({
        ...
        activeAgents: Object.values(runner.activeAgents || {}).map(entry => ({
          name: entry.name,
          model: entry.model || null,
          keyId: entry.keyId || null,
          visibility: entry.visibility || { mode: 'full', issues: [] },
    ...
    if (req.method === 'PATCH' && subPath.startsWith('agents/') && subPath.split('/')[1]) {
        ...
        try {
          ...
          const isManager = fs.existsSync(path.join(managersDir, `${normalizedAgentName}.md`));
          const isWorker = !RESERVED_MANAGER_NAMES.has(normalizedAgentName) && fs.existsSync(path.join(workersDir, `${normalizedAgentName}.md`));
          ...
          if (isManager) {
            ...
            if (!config.managers[normalizedAgentName]) config.managers[normalizedAgentName] = {};
            if (model) {
              config.managers[normalizedAgentName].model = model;
            } else {
              // Empty string = clear override (inherit from skill file)
              delete config.managers[normalizedAgentName].model;
              if (Object.keys(config.managers[normalizedAgentName]).length === 0) {
                delete config.managers[normalizedAgentName];
              }
            }
            fs.writeFileSync(runner.configPath, yaml.dump(config, { lineWidth: -1 }));
          } else {
...
// --- Main ---
log('TheBotGameCompany starting...');
checkPrerequisites();
syncProjects();
