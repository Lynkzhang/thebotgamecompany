...
import { listConfiguredMcpServers, listMcpTools, callMcpTool } from './mcp-client.js';
...
function translateShellPath(inputPath, cwd) {
  if (!inputPath || typeof inputPath !== 'string') return inputPath;
  if (os.platform() !== 'win32') return inputPath;

  if (inputPath === '/workspace') return cwd;
  if (inputPath.startsWith('/workspace/')) {
    return path.join(cwd, inputPath.slice('/workspace/'.length));
  }

  let match = inputPath.match(/^\/mnt\/([a-zA-Z])\/(.*)$/);
  if (match) {
    const [, drive, rest] = match;
    return `${drive.toUpperCase()}:\\${rest.replaceAll('/', '\\')}`;
  }

  match = inputPath.match(/^\/([a-zA-Z])\/(.*)$/);
  if (match) {
    const [, drive, rest] = match;
    return `${drive.toUpperCase()}:\\${rest.replaceAll('/', '\\')}`;
  }

  match = inputPath.match(/^([a-zA-Z])[\\/](Users|Program Files(?: \(x86\))?|Windows|Temp|tmp|workspace|repo)(?:[\\/](.*))?$/i);
  if (match) {
    const [, drive, firstSegment, rest = ''] = match;
    const suffix = rest ? `\\${rest.replaceAll('/', '\\')}` : '';
    return `${drive.toUpperCase()}:\\${firstSegment}${suffix}`;
  }

  return inputPath;
}

function windowsPathToPosixShellPath(targetPath) {
  if (!targetPath || typeof targetPath !== 'string') return targetPath;
  const match = targetPath.match(/^([A-Za-z]):[\\/](.*)$/);
  if (!match) return targetPath.replaceAll('\\', '/');
  const [, drive, rest] = match;
  return `/${drive.toLowerCase()}/${rest.replaceAll('\\', '/').replaceAll('//', '/')}`;
}

function rewriteCommandForShell(command, cwd, launcherCommand) {
  if (os.platform() !== 'win32') return command;
  if (!launcherCommand || /powershell(?:\.exe)?$/i.test(launcherCommand)) return command;

  let rewritten = command.replace(/\/mnt\/([a-zA-Z])\//g, (_, drive) => `/${drive.toLowerCase()}/`);
  rewritten = rewritten.replace(/\/workspace(?=\/|\b)/g, windowsPathToPosixShellPath(cwd));
  rewritten = rewritten.replace(/(["'])([A-Za-z]:\\[^"']*)\1/g, (_, quote, winPath) => `${quote}${windowsPathToPosixShellPath(winPath)}${quote}`);
  return rewritten;
}
...
function resolveToolPath(inputPath, cwd) {
  ...
  const translated = translateShellPath(inputPath, cwd);
  return path.isAbsolute(translated) ? normalizePath(translated) : normalizePath(path.join(cwd, translated));
}
...
function extractPathCandidates(command) {
  ...
  for (const match of quoted) {
    ...
    if (value && /^(\/|\.\.?[\/]|[A-Za-z]:[\\/]|\\\\|\\)/.test(value)) candidates.add(value);
  }

  const bare = command.match(/(?:^|\s)(\/[^\s|;&]+|\.\.?[\/][^\s|;&]+|[A-Za-z]:\\[^\s|;&]+|[A-Za-z]:\/[^\s|;&]+)/g) || [];
...
function resolveBashLauncher() {
  if (os.platform() !== 'win32') {
    return { command: 'bash', args: (cmd) => ['-c', cmd] };
  }
  ...
  const gitExe = process.env.ProgramFiles
    ...
    : null;
  const gitUsr = process.env.ProgramFiles
    ? path.join(process.env.ProgramFiles, 'Git', 'usr', 'bin', 'bash.exe')
    : null;
  const gitExeX86 = process.env['ProgramFiles(x86)']
    ? path.join(process.env['ProgramFiles(x86)'], 'Git', 'bin', 'bash.exe')
    : null;
  const gitUsrX86 = process.env['ProgramFiles(x86)']
    ? path.join(process.env['ProgramFiles(x86)'], 'Git', 'usr', 'bin', 'bash.exe')
    : null;

  for (const candidate of [gitExe, gitUsr, gitExeX86, gitUsrX86, ...candidates].filter(Boolean)) {
    if (fs.existsSync(candidate)) {
      return { command: candidate, args: (cmd) => ['-lc', cmd] };
    }
  }

  return { command: 'powershell.exe', args: (cmd) => ['-NoProfile', '-Command', cmd] };
}
...
function getToolDefinitions() {
  return [
    ...
    {
      name: 'Playwright',
      description: 'Run a short Playwright browser session for QA testing. Supports goto, click, fill, press, wait, and screenshot actions.',
      input_schema: {
        type: 'object',
        properties: {
          ...
          actions: {
            ...
            items: {
              type: 'object',
              properties: {
                type: { type: 'string', description: 'goto | click | fill | press | wait_for_selector | wait_for_timeout | screenshot' },
                selector: { type: 'string', description: 'CSS selector for click/fill/wait actions' },
                value: { type: 'string', description: 'Input text for fill, or screenshot path for screenshot action' },
                key: { type: 'string', description: 'Keyboard key for press action' },
                timeout: { type: 'number', description: 'Optional timeout in milliseconds for this action' },
              },
              required: ['type'],
            },
          },
          screenshot_path: { type: 'string', description: 'Optional output file path for a final screenshot' },
          headless: { type: 'boolean', description: 'Run headless browser (default true)' },
          timeout: { type: 'number', description: 'Overall page/action timeout in milliseconds (default 30000)' },
        },
        required: ['url'],
      },
    },
    {
      name: 'McpListTools',
      description: 'List available tools from configured MCP servers for this project.',
      input_schema: {
        type: 'object',
        properties: {
          server: { type: 'string', description: 'Optional MCP server name to inspect' },
        },
        required: [],
      },
    },
...
function executeBash(input, cwd, remainingMs = 0, bashEnv = null, runtime = null, allowedRepo = null, allowedPaths = null, issuePolicy = null) {
  ...
  return new Promise((resolve) => {
    ...
    command = rewriteCommandForShell(command, cwd, bashLauncher.command);
    let spawnCmd = bashLauncher.command;
    let spawnArgs = bashLauncher.args(command);
...
async function loadPlaywrightModule() {
  if (!playwrightModulePromise) {
    const modulePath = path.join(ROOT, 'monitor', 'node_modules', 'playwright', 'index.mjs');
    playwrightModulePromise = import(pathToFileURL(modulePath).href);
  }
  return playwrightModulePromise;
}

async function executePlaywright(input, cwd, allowedPaths = null) {
  ...
  try {
    ...
    for (const action of actions) {
      const actionTimeout = Math.min(action.timeout || timeout, 120000);
      switch (action.type) {
        case 'goto':
          await page.goto(action.value || action.url || input.url, { waitUntil: 'networkidle', timeout: actionTimeout });
          break;
        case 'click':
        ...
        case 'press':
          if (action.selector) {
            await page.press(action.selector, action.key || 'Enter', { timeout: actionTimeout });
          } else {
        ...
        case 'wait_for_timeout':
          await page.waitForTimeout(Math.min(Number(action.timeout || action.value || 1000), 30000));
          break;
        case 'screenshot': {
          let targetPath = action.value || input.screenshot_path;
          if (!targetPath) throw new Error('screenshot action requires value or screenshot_path');
          targetPath = resolveToolPath(targetPath, cwd);
          if (!isPathAllowed(targetPath, allowedPaths, 'write')) throw new Error(`access denied for ${targetPath}`);
    ...
    if (input.screenshot_path) {
      const targetPath = resolveToolPath(input.screenshot_path, cwd);
      if (!isPathAllowed(targetPath, allowedPaths, 'write')) throw new Error(`access denied for ${targetPath}`);
      fs.mkdirSync(path.dirname(targetPath), { recursive: true });
      await page.screenshot({ path: targetPath, fullPage: true });
    }
    ...
    return `Error running Playwright: ${err.message}`;
  } finally {
    try { await browser?.close(); } catch {}
  }
}

async function executeMcpListTools(input, projectConfig = null) {
  try {
    const servers = listConfiguredMcpServers(projectConfig || {});
    if (servers.length === 0) return 'No enabled MCP servers configured for this project.';
    const result = await listMcpTools(projectConfig || {}, input.server || null);
    return JSON.stringify(result, null, 2);
  } catch (err) {
    return `Error listing MCP tools: ${err.message}`;
  }
}

async function executeMcpCallTool(input, projectConfig = null) {
  try {
    const result = await callMcpTool(projectConfig || {}, input.server, input.tool, input.arguments || {});
    return JSON.stringify(result, null, 2);
  } catch (err) {
...
async function executeTool(toolName, toolInput, cwd, remainingMs = 0, bashEnv = null, runtime = null, allowedRepo = null, allowedPaths = null, issuePolicy = null, projectConfig = null) {
  switch (toolName) {
    ...
    case 'Playwright': return await executePlaywright(toolInput, cwd, allowedPaths);
    case 'McpListTools': return await executeMcpListTools(toolInput, projectConfig);
    case 'McpCallTool': return await executeMcpCallTool(toolInput, projectConfig);
...
// Auto-compact conversation history when approaching context limit
      ...
      if (response.stopReason === 'tool_use') {
        ...
        for (const tc of response.toolCalls) {
          ...
          const result = await executeTool(tc.name, tc.input, cwd, remainingMs, bashEnv, runtime, allowedRepo, allowedPaths, issuePolicy, projectConfig);
