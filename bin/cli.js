...
async function main() {
  switch (command) {
    case 'start':
      ...
      console.log(`TheBotGameCompany started (PID: ${child.pid})`);
    ...
    case 'dev':
      ensureHome();
      await ensureEnv();
      console.log('Starting TheBotGameCompany in development mode...\n');
    ...
    case 'status':
      try {
        ...
        console.log(`TheBotGameCompany - ${data.projectCount} projects`);
        ...
        console.log('TheBotGameCompany is not running');
      }
      break;
    ...
    case 'stop':
      {
        ...
        if (!fs.existsSync(pidFile)) {
          console.log('TheBotGameCompany is not running (no PID file)');
          break;
        }
        ...
        try {
          ...
          console.log(`Stopped TheBotGameCompany (PID: ${pid})`);
        } catch (e) {
          if (e.code === 'ESRCH') {
            fs.unlinkSync(pidFile);
            console.log('TheBotGameCompany was not running (stale PID)');
          } else {
    ...
    default:
      console.log(`TheBotGameCompany - Multi-project AI Agent Orchestrator
